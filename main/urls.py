...
from main.forms import LoginForm, RegistrationForm, EditProfileForm, PostForm, CommentForm
...
def explore():
    ...
    posts = Post.query.order_by(Post.timestamp.desc()).paginate(
        page, blog.config['POSTS_PER_PAGE'], False)
    next_url = url_for('blogs', page=posts.next_num) \
        if posts.has_next else None
    prev_url = url_for('blogs', page=posts.prev_num) \
        if posts.has_prev else None
    return render_template("index.html", title='Blogs', posts=posts.items,
                          next_url=next_url, prev_url=prev_url)
...
def post(id):
    ...
    if form.validate_on_submit():
        comment = Comment(body=form.comment.data, author=current_user, source=post)
        db.session.add(comment)
        db.session.commit()
        flash('Comment successful.')
        return redirect(url_for('post', id=id))
    comments = post.comments
    return render_template('idpost.html', post=post, form=form, comments=comments)